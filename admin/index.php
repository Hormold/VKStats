	...
	switch ($_GET["page"]) {
		...
		case 'new':
			if(isset($_POST["title"]) AND !empty($_POST["title"]) AND $_POST["link"]!==""){
				...
				if(isset($domain["host"])){
					...
					if($check==false){
						$db->insert(Array("title"=>$db->secure($_POST["title"]),"domain"=>$domain["host"],"uid"=>$user["id"],"its"=>time()),"projects");
						header("Location: /admin/");
					}
				}else{
		...
		case 'delete':
			if(isset($_GET["id"])){
				...
				if($check){
					if($db->update(Array("activate"=>0),"projects","id=".intval($_GET["id"]))){
						header("Location: /admin/");
					}
				}
			}
		...
		case 'actions.stats':
			if(isset($_GET["id"])){
				...
				if($check){
					...
					$actions_list=Array(); $byActions=Array();
					foreach($actions as $v){
						$uniqe=$db->sql("SELECT COUNT(DISTINCT user) FROM actions_log WHERE aid=".$v["action_id"],1);
						$byActions[]=Array("name"=>$v["name"],"uniqe"=>$uniqe[0]["COUNT(DISTINCT user)"],"total"=>$db->count("actions_log","aid=".$v["action_id"]));
					}
		...
		case 'actions.daily':
			if(isset($_GET["id"])){
				...
				if($check){
					...
					$d=array_reverse($d);$range=array_reverse($range);$table_days=array_reverse($table_days);
					...
					foreach($actions as $action){
						...
						$data[]=Array("name"=>$action["name"],"data"=>$query,"data2"=>$query2);
						$data2[]=Array("name"=>$action["name"],"data"=>$query2);
					}
		...
		case 'actions.delete':
			if(isset($_GET["id"])){
				...
				if($check && $check2){
					if($db->update(Array("active"=>0),"actions","action_id=".intval($_GET["id"]))){
			...
			if(isset($_GET["id"]) AND isset($_POST["title"]) AND !empty($_POST["title"]) AND $_POST["type"]!==""){
				...
				if($type=="redirect" OR $type=="action"){
					if($db->select("actions","active=1 AND name='".$db->secure($_POST["title"])."'",1)==false AND $db->select("projects","uid=".$user["id"]." AND id='".$db->secure($_GET["id"])."'",1)!==false){
						...
						if($check2){
							$db->insert(Array("name"=>$db->secure($_POST["title"]),"type"=>$type,"pid"=>$_GET["id"],"value"=>$db->secure($_POST["value"]),"target_action"=>(isset($_POST["target_action"]) AND $_POST["target_action"]=="on")?1:0),"actions");
